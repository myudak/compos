...
export function TransactionDetailPage() {
  ...
  const events = [
    ...
    {
      label: "Disimpan ke local outbox",
      description: "Aman dari refresh dan restart browser",
      done: true,
    },
    {
      ...
      description: transaction.receivedAtBackend
        ...
        : transaction.syncStatus === "FAILED"
          ? (transaction.lastSyncError ?? "Retry diperlukan")
          : "Menunggu koneksi",
      done: transaction.settlementStatus === "SETTLED",
    ...
    },
    {
      ...
      description:
        transaction.settlementStatus === "SETTLED"
          ? "Histori terkunci; koreksi membuat record baru"
          : "Menunggu acceptance backend",
      done: transaction.settlementStatus === "SETTLED",
    },
  ]
  ...
  async function voidSale() {
    try {
      ...
      toast.success("Transaksi di-void", {
        description: "Void akan disinkronkan sebagai histori; stok lokal telah dikembalikan.",
      })
    } catch (error) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              {formatTransactionDate(transaction.createdAt)} · Kedai Nusa / Blok M
            </p>
          </div>
        </div>
              ...
              {transaction.amountReceived && (
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-md bg-secondary p-2.5">
                    <div className="text-[9px] text-muted-foreground">Diterima</div>
                    <div className="mt-1 text-xs font-semibold">
                      {formatCurrency(transaction.amountReceived)}
                    </div>
                  </div>
                  <div className="rounded-md bg-secondary p-2.5">
                    <div className="text-[9px] text-muted-foreground">Kembalian</div>
          ...
          >
            <IconLock
              className={cn(
                ...
                transaction.settlementStatus === "SETTLED" ? "text-emerald-400" : "text-amber-300",
              )}
            />
            <div>
              ...
              <p className="mt-1 text-[10px] leading-4 text-muted-foreground">
                {transaction.settlementStatus === "SETTLED"
                  ? "Operator tidak dapat mengubah transaksi yang sudah settled. Koreksi hanya dapat dibuat admin sebagai record audit baru."
                  : "Transaksi aman di perangkat, tetapi belum menjadi histori final sampai backend menerimanya."}
              </p>
            </div>
          </div>
        </div>
      ...
      <Dialog open={voidOpen} onOpenChange={setVoidOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Item akan dikembalikan ke proyeksi stok lokal. Record void tetap disinkronkan agar
              audit history tidak hilang.
            </DialogDescription>
          </DialogHeader>
