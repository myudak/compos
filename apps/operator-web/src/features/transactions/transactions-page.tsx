  ...
  return (
    <div>
      <PageHeader
        ...
        title="Transaksi"
        description="Semua penjualan dari perangkat ini. Status sync dan settlement selalu terlihat—termasuk saat offline."
        actions={
          <Button variant="outline">
            <IconFilter /> Export shift
          </Button>
        }
      />
      ...
      <div className="grid gap-px border-b bg-border sm:grid-cols-3">
        ...
        <div className="bg-background px-4 py-3 sm:px-6">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Settled</div>
          <div className="mt-1 flex items-center gap-2 text-lg font-semibold">
            <IconCloudCheck className="size-4 text-emerald-400" />{" "}
            {transactions.length - provisional}
          </div>
        </div>
        <div className="bg-background px-4 py-3 sm:px-6">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <div
            className={cn(
              ...
              provisional > 0 && "text-amber-300",
            )}
          >
            <IconClock className="size-4" /> {provisional}
          </div>
        </div>
      </div>
            ...
            >
              ...
              {item.value !== "ALL" && (
                <span className="ml-1 rounded bg-background px-1 text-[9px] text-muted-foreground">
                  {
                    transactions.filter((transaction) => matchesFilter(transaction, item.value))
                      .length
                  }
                </span>
              )}
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((transaction) => (
                <tr
                  key={transaction.id}
                  className="group bg-card/40 transition-colors hover:bg-accent/60"
                >
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">{transaction.invoiceNumber}</span>
                      {transaction.transactionStatus === "VOIDED" && (
                        <span className="rounded bg-zinc-500/15 px-1.5 py-0.5 text-[8px] font-bold uppercase text-zinc-400">
                          Voided
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-3 py-3">
                    <div>{paymentLabels[transaction.paymentMethod]}</div>
      ...
      <div className="grid gap-2 p-4 sm:hidden">
        {filtered.map((transaction) => (
          <Link key={transaction.id} to={`/transactions/${transaction.id}`}>
            <Card className="grid gap-3 p-3 active:bg-accent">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-muted-foreground">
                  {paymentLabels[transaction.paymentMethod]} · {transaction.items.length} jenis
                </span>
      ...
      {filtered.length === 0 && (
        <div className="grid min-h-80 place-items-center text-center">
          <div>
            <IconReceipt2 className="mx-auto mb-2 size-7 text-muted-foreground" />
