...
export function registerDeviceRoutes(app: FastifyInstance, pool: DatabasePool) {
  app.post("/v1/devices/register", async (request, reply) => {
    ...
    if (body.activationCode !== config.DEVICE_ACTIVATION_CODE)
      return reply.code(403).send({ code: "INVALID_ACTIVATION_CODE" })
    const merchant = await pool.query<{ id: string }>("SELECT id FROM merchants WHERE code = $1", [
      body.merchantCode,
    ])
    ...
    return reply
      .code(201)
      .send({ deviceId: body.deviceId, merchantId: merchant.rows[0].id, status: "REGISTERED" })
  })
  ...
  app.post("/v1/devices/:deviceId/revoke", async (request, reply) => {
    ...
    const result = await pool.query(
      "UPDATE devices SET revoked_at = now() WHERE id = $1 AND merchant_id = $2 AND revoked_at IS NULL",
      [params.deviceId, identity.merchantId],
    )
