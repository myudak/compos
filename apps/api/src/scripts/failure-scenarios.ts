...
import { processBackendOutbox } from "../worker.js"
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) throw Object.assign(new Error(`${response.status} ${JSON.stringify(body)}`), { status: response.status, body })
  return body
}

async function register(deviceId: string) {
  await request("/v1/devices/register", { method: "POST", body: JSON.stringify({ merchantCode: "KEDAI-NUSA", activationCode: "COMP18-DEMO", deviceId, deviceName: `Failure ${deviceId.slice(-6)}` }) })
}

async function login(deviceId: string, operatorCode = "RANI", pin = "1234") {
  return request<{ token: string; operator: { id: string }; merchantId: string }>("/v1/auth/login", { method: "POST", body: JSON.stringify({ merchantCode: "KEDAI-NUSA", operatorCode, pin, deviceId }) })
}
...
async function sync(token: string, merchantId: string, deviceId: string, transactions: unknown[]) {
  return request<{ results: Array<{ transactionId: string; status: string; reason?: string }> }>("/v1/sync/transactions", {
    method: "POST",
...
const lostResponseRetry = await sync(sessionA.token, sessionA.merchantId, deviceA, [lostResponseTransaction])
...
const crossDeviceRetry = await sync(sessionB.token, sessionB.merchantId, deviceB, [lostResponseTransaction])
expectStatus(crossDeviceRetry.results[0]?.status, "ALREADY_PROCESSED", "cross-device idempotency")
...
const partial = await sync(sessionA.token, sessionA.merchantId, deviceA, [validSibling, invalidSibling])
...
const burst = Array.from({ length: 30 }, (_, index) => transaction(sessionA.operator.id, `burst-${index}`))
const burstResults = [
...
if (burstResults.some((result) => result.status !== "ACCEPTED")) throw new Error("bounded reconnect burst was not fully accepted")
...
const correction = await request<{ correctionId: string; originalTransactionMutated: boolean }>(`/v1/admin/transactions/${lostResponseTransaction.transactionId}/corrections`, {
  method: "POST",
  headers: { authorization: `Bearer ${admin.token}` },
  body: JSON.stringify({ reason: "Automated reconciliation verification", adjustmentAmount: -1_000, evidenceReference: "TEST-EVIDENCE" }),
})
if (correction.originalTransactionMutated !== false) throw new Error("correction mutated the settled transaction")
...
const discrepancyResolution = await request<{ status: string }>(`/v1/inventory/discrepancies/${openDiscrepancy.id}/resolve`, {
  method: "POST",
  headers: { authorization: `Bearer ${admin.token}` },
  body: JSON.stringify({ resolution: "Physical stock counted during automated verification", adjustedStock: 0 }),
})
expectStatus(discrepancyResolution.status, "RESOLVED", "inventory discrepancy resolution")

await request(`/v1/devices/${deviceB}/revoke`, { method: "POST", headers: { authorization: `Bearer ${admin.token}` } })
let revokedStatus = 0
try {
  await sync(sessionB.token, sessionB.merchantId, deviceB, [transaction(sessionB.operator.id, "revoked")])
} catch (error) {
  revokedStatus = (error as { status?: number }).status ?? 0
}
if (revokedStatus !== 403) throw new Error(`revoked device: expected HTTP 403, received ${revokedStatus}`)
